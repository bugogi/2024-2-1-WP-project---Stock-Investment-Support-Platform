...
<body>
    ...
    <article>
        <h2 id="page_name">Trend Stocks</h2>
        <?php
        //(로그인 시)현재 잔액
        if ($_SESSION["authuser"] == 1) {
        ...
        ?>
        <!--전문가 ryu 추천 주식들-->
        <h3>RYU expert's recommendation</h3>
        <section class="ryu_stocks">
            <?php
            //전문가 ryu 추천 주식 쿼리 생성
            $query_stock = "SELECT * FROM stocks WHERE rec_ryu = 1";
        ...
        <!--전문가 WOO 추천 주식들-->
        <h3>WOO expert's recommendation</h3>
        <section class="woo_stocks">
            <?php
            //전문가 woo 추천 주식 쿼리 생성
            $query_stock = "SELECT * FROM stocks WHERE rec_woo = 1";
            $result_stock = mysqli_query($db, $query_stock);
            //각 주식
            while ($row = $result_stock->fetch_assoc()) {
                echo "<div class=\"stock_card\">";
