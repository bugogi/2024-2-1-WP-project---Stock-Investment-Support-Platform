...
//로그인 페이지에서 로그인 시도 했을 시
if(isset($_POST['source']) && $_POST['source'] == 'loginpage'){
    $_SESSION['usersubmit'] = $_POST['login'];
    $_SESSION['userid'] = $_POST['id'];
    $_SESSION['userpass'] = $_POST['password'];
}else{
    $_SESSION['usersubmit'] = NULL;
}

//사용자 권한 확인
if(($_SESSION['usersubmit'] == NULL) and ($_SESSION['authuser'] != 1)
    or isset($_POST['logout'])){
    $_SESSION['authuser'] = 2;//로그인 성공 1 로그인 실패 0 게스트모드 2
} elseif ((($_SESSION['userid'] == 'Heo') and($_SESSION['userpass'] == '0124')) 
            or ($_SESSION['authuser'] == 1)
) {
    $_SESSION['authuser'] = 1;
} else {
    $_SESSION['authuser'] = 0;
...
if($_SESSION['authuser'] == 0){
    echo "<script>window.alert(\"Sorry, but you don\'t have permission to view this page!\");</script>";
    echo "<script>location.href='WP project_login.php'</script>";
}
...
<html lang="en">
    ...
    <body>
        <header>
            ...
            <div>
                <nav><!--네비게이션 국내, 해외, 트렌드-->
                    <a href="WP project_domestic.php" style="text-decoration: underline;">Domestic Stocks</a>
        ...
        <article>
            <h2 id="page_name">Domestic Stocks</h2>
            ...
            <?php
                ...
                //국내 주식 쿼리
                $query_stock = "SELECT * FROM stocks WHERE dom_or_over = 0";
            ...
            <section class="stocks">
                <?php
                    //주식 이름, 현재 가격, 주식 정보, (로그인시)보유 개수, 구매, 판매, 좋아요
                    while($row = $result_stock->fetch_assoc()) {
                        ...
                        echo ">♥</button>";
                        echo "<p id=\"like_num_" . $row["stock_ID"] . "\">" . $row["like_num"] . "</p>";
                        echo "</div>";
                        echo "</form>";
                        echo "</div>";
                    }
